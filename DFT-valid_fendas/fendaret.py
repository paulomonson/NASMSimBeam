...
x0 = 0  
y0=0                        # Centro da gaussiana e da fenda
largura_fendax = 0.0007
largura_fenday = 0.0009
...
def feixe_nafenda(E0,X, Y, x0, y0, largura_fendax, largura_fenday):
    mascara_fenda = np.where((np.abs(X - x0) <= largura_fendax/2) & (np.abs(Y - y0) <= largura_fenday/2), 1, 0)
...
mascara_fenda, E_nafenda= feixe_nafenda(E0,X, Y, x0, y0, largura_fendax, largura_fenday)
       
plt.imshow(mascara_fenda)
plt.suptitle(f'fenda em x com = {largura_fendax*100} cm , fenda em y com = {largura_fenday*100} cm')
...
ax_mp1.set_title('Intensity em z=0 (Origem)')
...
heatmap2 = ax_mp2.imshow(I_zfinal, extent=[y.min(), y.max(), x.max(), x.min()], cmap='hot', vmin=0, vmax=0.04)
ax_mp2.set_title(f'Intensity em z={z} m (Final)')
